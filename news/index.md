...
- Bump the `extendr` crates to 0.9.0, which drops the
  `R_NamespaceRegistry` binding. This clears the `R CMD check` NOTE
...
- Migrate the package scaffolding to `rextendr` 0.5.0.
  `R/extendr-wrappers.R` is now generated during the build by a small
  `document` binary (`src/rust/document.rs`) rather than by
  [`rextendr::register_extendr()`](https://extendr.github.io/rextendr/reference/register_extendr.html),
  and the package gains `cleanup`/`cleanup.win`. `src/entrypoint.c` now
  registers extendr’s panic hook.
- Move the `@noRd`/`@keywords internal` roxygen tags for
  `sql_format_wrapper` into the Rust doc comment, so they survive
