      ...
      devShells = forAllSystems (pkgs: {
        default =
          let
            ...
            # used by the justfile recipes. renv is included so `renv::restore()`
            # still works for anything pinned in renv.lock.
          ...
          pkgs.mkShell {
            ...
            # Headers/libraries for R packages that renv may build from source.
            ++ (with pkgs; [
              curl
              fontconfig
              freetype
            ...
            shellHook = ''
              # Keep renv's cache and any renv-installed packages inside the
              # project so they survive between shells without touching $HOME.
              export RENV_PATHS_ROOT="$PWD/.renv"
